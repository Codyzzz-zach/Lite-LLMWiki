...
raw (.md / .tex / TeX 文件夹 / .pdf)
  → [load] Source {chunks, fingerprint} → PDF 额外走 Pro 清洗
...
│   └── prefix.ts         # 三模式 system prompt + 四层前缀
├── ingest/
│   ├── loader.ts         # MD 加载
│   ├── tex-loader.ts     # LaTeX 文件夹/单文件 → Pro 清洗 → MD
│   ├── pdf-loader.ts     # PDF → pdf-parse → Pro 清洗 → MD
...
- ✅ .bbl 模糊匹配、缺失子文件 warn、递归深度上限
- ✅ TEX_CLEAN_PROMPT 支持 theorem/thanks/itemize
- ✅ 模型: deepseek-v4-pro
