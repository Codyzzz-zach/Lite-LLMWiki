	...
	switch (policy) {
		...
		case "conservative": {
			...
			if (!kind || !FACT_KINDS.has(kind)) {
				return {
					accept: false,
					reason: `Conservative: kind "${kind ?? "undefined"}" not in fact types`,
				};
			}
			// 必须 high confidence
			if (confidence < HIGH_CONF) {
				return {
					accept: false,
					reason: `Conservative: confidence ${confidence.toFixed(2)} < ${HIGH_CONF}`,
				};
			}
			// 必须 evidence 完整
		...
		case "balanced": {
			...
			if (kind && SOFT_KINDS.has(kind)) {
				if (!evidence || evidence.length === 0) {
					return {
						accept: false,
						reason: `Balanced: "${kind}" needs evidence`,
					};
				}
				return { accept: true };
			}
			...
			return {
				accept: false,
				reason: `Balanced: kind "${kind ?? "undefined"}" not auto-confirmable`,
			};
		}
		...
		case "expansive": {
			...
			if (kind && LIMITS_REQUIRED_KINDS.has(kind)) {
				if (!limits || limits.length === 0) {
					return {
						accept: false,
						reason: `Expansive: "${kind}" must have limits`,
					};
				}
				return { accept: true };
			}
