...
export async function loadFromPdf(
	...
	if (!applyRes.ok) {
		...
		throw new Error(
			`Agent apply failed (${applyRes.status}): ${err.slice(0, 300)}`,
		);
	}
	const applyData = (await applyRes.json()) as any;
	if (applyData.code !== 0) {
		throw new Error(
			`Agent apply error: ${applyData.msg ?? "unknown"} (code=${applyData.code})`,
		);
	}
	...
	if (!taskId || !fileUrl) {
		throw new Error(
			`Agent: no task_id/file_url: ${JSON.stringify(applyData).slice(0, 300)}`,
		);
	}
	...
	if (!markdownUrl)
		throw new Error(`Agent: timeout — no result within ${MAX_WAIT / 1000}s`);
