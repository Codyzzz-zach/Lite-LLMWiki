...
import {
	buildFormatStepPrefix,
	buildIngestPrefix,
	buildThinkStepPrefix,
} from "../core/prefix.js";
import type {
	AppConfig,
	Evidence,
...
export async function proIngest(opts: IngestOpts): Promise<ProResult> {
	...
	if (mode === "extract") {
		...
		console.error(
			`  [Pro] extract — step 1 done (${thinkResult.usage?.completionTokens ?? "?"} tokens)`,
		);

		// Step 2: Flash 结构化提取（严格 JSON）
		const formatPrompt = buildFormatStepPrefix(
			prefixOpts,
			"format-extract",
			thinkResult.content,
		);
	...
	if (mode === "compile") {
		...
		console.error(
			`  [Pro] compile — step 1 done (${thinkResult.usage?.completionTokens ?? "?"} tokens)`,
		);
...
function parseProResult(
	...
	if (mode === "extract") {
		...
		const mainThreads: MainThread[] = rawThreads.map((t) => ({
			...
			chunkRefs: clampChunkRefs(
				Array.isArray(t.chunkRefs) ? (t.chunkRefs as number[]) : [],
				maxChunkRef,
			),
		}));
		...
		const propositions: Proposition[] = rawProps.map((p) => {
			const chunkRefs = clampChunkRefs(
				parseNumberArray(p.chunkRefs),
				maxChunkRef,
			);
			return {
				id: asNumber(p.id, 0),
				...
				counterIntuitiveReason:
					(p.counterIntuitiveReason as string) ?? undefined,
				kind: parseWikiKind(p.kind),
				evidence: normalizeEvidenceArray(
					p.evidence,
					source.id,
					chunkRefs,
					maxChunkRef,
	...
	if (mode === "reread") {
		...
		const rawChunkRefs = clampChunkRefs(
			parseNumberArray(p?.chunkRefs),
			maxChunkRef,
		);
	...
	// compile
	const rawNodeDrafts = Array.isArray(parsed.nodeDrafts)
		? (parsed.nodeDrafts as Record<string, unknown>[])
		: [];
	const fallbackSlug = source.title
		.replace(/[^a-zA-Z0-9一-鿿]/g, "-")
		.replace(/-+/g, "-")
		.toLowerCase()
		.slice(0, 40)
		.replace(/^-|-$/g, "");
	const parsedNodeDrafts: import("../types.js").WikiNodeDraft[] =
		rawNodeDrafts.map((d) => {
			const rawFm = isRecord(d.frontmatter) ? d.frontmatter : {};
			const nodeId =
				(d.nodeId as string) ??
				(rawFm.nodeId as string) ??
				`concept/${fallbackSlug}`;
			const kind = parseWikiKind(d.kind ?? rawFm.kind) ?? "concept";
			const evidence = normalizeEvidenceArray(
				d.evidence,
				source.id,
				parseNumberArray(d.chunkRefs ?? rawFm.chunkRefs),
				maxChunkRef,
			);
			const propRefs = uniqueStrings([
				...parsePropRefs(rawFm),
				...parsePropRefs(d as Record<string, unknown>),
				...evidence.flatMap((ev) => ev.propRefs),
			]);
			const frontmatter: WikiFrontmatter = {
				...
				propRefs,
				confidence:
					parseConfidence(rawFm.confidence ?? d.confidence) ??
					(evidence.length > 0 ? 0.7 : 0.3),
				status:
					parseStatus(rawFm.status) ??
					(evidence.length > 0 ? "verified" : "needs_review"),
	...
	const rawUpdated = Array.isArray(parsed.updatedPages)
		? (parsed.updatedPages as Record<string, unknown>[])
		: [];
	const updatedPages: WikiPage[] = rawUpdated.map((p) => ({
		...
		frontmatter: {
			...(((p.frontmatter as Record<string, unknown>) ??
				{}) as unknown as WikiFrontmatter),
			title:
				((p.frontmatter as Record<string, unknown>)?.title as string) ??
				source.title,
		},
...
function fallbackResult(
	...
	return {
		...
		mode: "extract",
		mainThreads: [
			{ id: 1, title: "全文", description: "自动化提取", chunkRefs: [] },
		],
		propositions: [
			{
				id: 1,
...
/** 从 LLM 输出中解析 propRefs：优先读 propRefs（新 prompt），回退 chunkRefs（旧 prompt） */
function parsePropRefs(raw: Record<string, unknown>): string[] {
	const props = raw.propRefs;
	if (Array.isArray(props)) {
		return props.map((v) => String(v)).filter((s) => s.trim().length > 0);
	}
	return [];
}
...
function normalizeEvidenceArray(
	...
	return value
		.filter(isRecord)
		.map((item) => {
			const rawPropRefs = parsePropRefs(item);
			const rawChunkRefs = rawPropRefs
				.map(Number)
				.filter((n) => Number.isFinite(n));
			const chunkRefs = clampChunkRefs(
				uniqueNumbers([...rawChunkRefs, ...fallbackChunkRefs]),
				maxChunkRef,
			);
			return {
				sourceId: (item.sourceId as string) ?? fallbackSourceId,
				propRefs: chunkRefs.map(String),
				excerpt:
					(item.excerpt as string) ?? (item.quote as string) ?? undefined,
				quote: (item.quote as string) ?? undefined,
				summary: (item.summary as string) ?? undefined,
			};
		})
		.filter(
			(item) => item.sourceId.trim().length > 0 && item.propRefs.length > 0,
		);
}
...
function normalizeWikiFilePath(
	value: unknown,
	kind: WikiKind,
	nodeId: string,
): string {
...
function uniquifyNodeDraftPaths<T extends { nodeId: string; filePath: string }>(
	drafts: T[],
): T[] {
	...
	return drafts.map((draft) => {
		...
		const nodeId = draft.nodeId.endsWith(suffix)
			? draft.nodeId
			: `${draft.nodeId}${suffix}`;
