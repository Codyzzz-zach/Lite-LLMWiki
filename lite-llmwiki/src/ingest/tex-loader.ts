...
function resolveTexIncludes(
	...
	if (depth > MAX_RECURSION_DEPTH) {
		throw new Error(
			`TeX \`\\input{} 递归深度超过 ${MAX_RECURSION_DEPTH} 层，可能是循环引用`,
		);
	}
	...
	return content.replace(/\\(?:input|include)\{(.+?)\}/g, (_, name: string) => {
		...
		console.warn(
			`  [TeX] ⚠️  missing: ${cleanName}（未找到子文件，对应内容已跳过）`,
		);
	...
	return rawTex.replace(/\\(?:bibliography)\{(.+?)\}/g, (_, name: string) => {
		...
		try {
			...
			if (files.length > 0) {
				...
				console.warn(
					`  [TeX] ⚠️  \`\\bibliography{${cleanName}} 未找到，使用 ${files[0]} 替代`,
				);
				return fallback;
			}
		} catch {
...
export async function loadFromTex(
	filePath: string,
	config?: AppConfig,
	options?: {
	...
	if (estTokens > 50000) {
		console.warn(
			`  [TeX] ⚠️  论文较长（~${(estTokens / 1000).toFixed(0)}K tokens），清洗可能较慢`,
		);
	}
	...
	try {
		...
		console.error(
			`  [TeX] cleaning failed, falling back to raw .tex: ${(err as Error).message}`,
		);
		body = resolvedTex;
	}
	...
	const chunks = rawChunks.map((text, index) => {
		...
		const end = Math.min(
			body.length,
			(start >= 0 ? start : charPos) + text.length,
		);
