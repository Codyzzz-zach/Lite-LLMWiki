...
export interface SemanticAuditOptions {
   ...
   * 不传时回退到占位实现（不调真实 LLM，每个 node 记 warning）。
   */
...
async function auditOneNode(
  ...
  const chunkRefs = node.frontmatter.chunkRefs ?? [];

  // ── 早退：chase 缺失（spec 7.7：直接 error，不调 LLM） ──
  const chasePath = resolveChasePath(config, sourceChase);
  if (!chasePath) {
    return failedNoLLM(node, "citation", `chase file not found for sourceChase: ${sourceChase.join(", ") || "(empty)"}`, issues);
  }

  // ── 早退：chunkRefs 缺失（spec 7.7） ──
  if (chunkRefs.length === 0) {
    return failedNoLLM(node, "citation", "missing chunkRefs", issues);
  }

  // ── 读 chase excerpts ──
  let excerpts: { index: number; text: string }[];
  try {
    const result = selectChaseChunks(config, sourceChase, chunkRefs);
  ...
  try {
    const judge = options.llmJudge ?? placeholderJudge;
    rawResponse = await judge(prompt);
  } catch (e) {
...
function emitVerdictIssues(
  ...
  const flag = (dim: AuditDimension, value: string, defaultReason: string, suggestedFix?: string) => {
  ...
  for (const issueText of verdict.issues) {
    ...
    const severity: "warning" | "error" = verdict.verdict === "failed" ? "error" : "warning";
    issues.push({
      nodeId: node.nodeId,
      filePath,
      severity,
...
function inferDimension(text: string): AuditDimension {
  ...
  if (/chunkref|chunk ref|citation|引用.*不覆盖/.test(lower)) return "citation";
...
function collectNodes(config: AppConfig, options: SemanticAuditOptions): ParsedWikiNode[] {
  const paths: string[] = [];
