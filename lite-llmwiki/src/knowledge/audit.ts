...
import { collectPropIndices } from "./chase.js";
...
export function auditWiki(
	...
	for (const { filePath, fullPath } of files) {
		...
		// ── Check: propRefs ──
		const propRefs = parsed.frontmatter.propRefs ?? [];
		if (propRefs.length === 0) {
			...
			issues.push({
				...
				filePath,
				nodeId: parsed.nodeId,
				message: "Missing propRefs",
			});
		}

		if (propRefs.length > 0 && chaseFileContent) {
			const validIndices = collectPropIndices(chaseFileContent);
			for (const ref of propRefs) {
				const numRef = Number(ref);
				if (!isNaN(numRef) && !validIndices.has(numRef)) {
					invalidChunkRefs++;
					issues.push({
						severity: "error",
						filePath,
						nodeId: parsed.nodeId,
						message: `Invalid propRef: ${ref} — not found in chase file`,
					});
				}
			}
		}
		...
		const nodeHasError = issues.some(
			(issue) => issue.severity === "error" && issue.filePath === filePath,
		);
...
export function writeAuditResults(
	config: AppConfig,
	result: AuditResult,
): void {
