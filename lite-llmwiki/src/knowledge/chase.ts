...
import type { AppConfig, ChaseChunk } from "../types.js";
 ...
 * `<rawDir>/chase/x.md`。
 */
export function resolveChasePath(config: AppConfig, sourceChase: string[]): string | null {
 ...
 * - 文件不可读 → 抛原始 IO 错误
 */
export function readChaseChunks(config: AppConfig, sourceChase: string[]): ChaseChunk[] {
