...
export function renderWikiNode(draft: WikiNodeDraft): string {
	...
	if (!evidence || evidence.length === 0) {
		...
		for (const ev of evidence) {
			const refs =
				ev.propRefs.length > 0
					? ` | PropRefs: [${ev.propRefs.join(", ")}]`
					: "";
...
export function normalizeFrontmatter(
	...
	const propRefs = unique([
		...(draft.frontmatter.propRefs ?? []),
	...
	const normalized: ValidatedWikiFrontmatter = {
		...
		confidence:
			draft.frontmatter.confidence ?? (evidence.length > 0 ? 0.7 : 0.3),
		...
		updatedAt:
			draft.frontmatter.updatedAt ?? draft.frontmatter.createdAt ?? now,
	};
...
function validateFrontmatter(
	...
	if (fm.propRefs.length === 0) errors.push("propRefs must not be empty");
	if (!draft.claim || draft.claim.trim().length === 0)
		errors.push("claim must not be empty");
	if (errors.length > 0) {
		throw new Error(
			`Invalid WikiNodeDraft "${draft.nodeId}": ${errors.join("; ")}`,
		);
	}
}
