...
export function buildSemanticAuditInput(
	...
	return {
		...
		propRefs: node.frontmatter.propRefs ?? [],
		chaseExcerpts,
	};
}
...
export function buildSemanticAuditPrompt(input: SemanticAuditInput): string {
	...
	parts.push(``);
	parts.push(
		`You are auditing a wiki node for semantic faithfulness to its source (chase) excerpts.`,
	);
	parts.push(
		`Do NOT evaluate whether the source is true. Only evaluate whether the wiki node is faithful to the chase excerpts.`,
	);
	parts.push(``);
	...
	if (input.propRefs.length === 0) {
		parts.push(`(missing propRefs — citation cannot be verified)`);
	} else {
		parts.push(`Requested propRefs: [${input.propRefs.join(", ")}]`);
	}
	parts.push(``);
	...
	parts.push(
		`- **support**: Does the evidence support the claim? (aligned | stretched | unsupported)`,
	);
	parts.push(
		`- **addition**: Does the wiki add claims not in source? (none | minor | major)`,
	);
	parts.push(
		`- **inference**: Is inference correctly marked as inference? (ok | warning | failed)`,
	);
	parts.push(
		`- **limits**: Are important source conditions preserved in Limits? (ok | warning | failed)`,
	);
	parts.push(
		`- **citation**: Do propRefs cover the key evidence? (ok | warning | failed)`,
	);
	parts.push(``);
	...
	parts.push(
		`Output JSON only (no prose, no markdown fence). Use this exact shape:`,
	);
	parts.push(`{`);
	...
	parts.push(
		`  "anchor": "prop N: 'exact quote from source that supports your verdict'"`,
	);
	parts.push(`}`);
	parts.push(``);
	parts.push(
		`verdict guidance: passed = no issues; warning = minor issues; failed = the wiki distorts the source.`,
	);
...
const VALID_SUPPORT: SemanticVerdict[] = [
	"aligned",
	"stretched",
	"unsupported",
];
...
export function parseSemanticAuditResponse(
	...
	if (typeof verdict !== "string" || !VALID_VERDICTS.has(verdict)) {
		throw new Error(
			`semantic audit response has invalid verdict: ${String(verdict)}`,
		);
	}
	...
	if (typeof score !== "number" || score < 0 || score > 1) {
		throw new Error(
			`semantic audit response has invalid score: ${String(score)}`,
		);
	}
	...
	if (
		...
		!VALID_SUPPORT.includes(support as SemanticVerdict)
	) {
		throw new Error(
			`semantic audit response has invalid support: ${String(support)}`,
		);
	}
	...
	if (typeof addition !== "string" || !VALID_ADDITION.has(addition)) {
		throw new Error(
			`semantic audit response has invalid addition: ${String(addition)}`,
		);
	}
	...
	if (Array.isArray(issuesRaw)) {
		...
		throw new Error(
			`semantic audit response has invalid issues: ${String(issuesRaw)}`,
		);
	}
