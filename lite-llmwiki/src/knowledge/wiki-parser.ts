...
export function parseWikiContent(content: string, filePath: string): ParsedWikiNode {
  ...
  return {
    ...
    title,
    filePath,
    frontmatter: normalizeFrontmatter(fm, kind, title, nodeId, sections.Claim ?? ""),
    sections: {
...
function normalizeFrontmatter(
  ...
  if (chunkRefs.length > 0) out.chunkRefs = chunkRefs;
  ...
  if (status === "draft" || status === "verified" || status === "needs_review" || status === "legacy") {
    out.status = status;
  }
...
function parseBoardRoles(raw: string | string[] | undefined): BoardRole[] | undefined {
...
function parseInferenceLevel(s: string | undefined): InferenceLevel | undefined {
  if (!s) return undefined;
  const valid: InferenceLevel[] = ["none", "light", "medium", "strong"];
  return valid.includes(s as InferenceLevel) ? (s as InferenceLevel) : undefined;
}
