...
基于用户已编译的 board 局面，生成启发项（connections, hypotheses, questions, actions, missingEvidence）。

用户消息中会包含 board 的结构化内容（种子节点、证据、反直觉视角、局限、知识缺口等）。
你必须基于这些 board 内容来生成启发，而不是凭自己的训练数据凭空创造。
...
function serializeBoardToPrompt(board: QueryBoard): string {
  ...
  lines.push(`种子节点: ${board.seedNodes.length} | 证据节点: ${board.evidenceNodes.length} | 相关节点: ${board.relatedNodes.length}`);
  lines.push(`反直觉节点: ${board.counterNodes.length} | 局限节点: ${board.limitNodes.length} | 问题节点: ${board.questionNodes.length}`);
  if (board.gaps.length > 0) {
  ...
  for (const n of board.seedNodes) {
    ...
    if (n.evidence.length > 0) lines.push(`  evidence: ${n.evidence.join("; ")}`);
  ...
  if (board.sourceExcerpts.length > 0) {
    ...
    for (const ex of board.sourceExcerpts) {
      lines.push(`[source: ${ex.sourceId}, chunk ${ex.chunkRefs.join(",")}]`);
