...
export async function buildQueryBoard(
  ...
  const sourceExcerpts = mode === "trace" || options.withSource
...
async function findSeedNodes(
  ...
  // --source 过滤
  if (options.source) {
    seeds = seeds.filter((n) => n.sourceIds.some((s) => s.includes(options.source!)));
  }
...
function loadNodeById(config: AppConfig, nodeId: string): ParsedWikiNode | null {
  for (const dir of WIKI_NODE_DIRS) {
    const dirPath = join(config.wikiDir, dir);
    if (!existsSync(dirPath)) continue;
    for (const f of readdirSync(dirPath)) {
...
async function assembleBoard(
  ...
  switch (mode) {
    case "ask": {
      ...
      const evidenceNodes = pickEvidence(seeds, allBoardNodes, seedIds, /* max */ 5);
      const relatedNodes = pickRelated(seeds, allBoardNodes, seedIds, /* max */ 3);
    ...
    case "trace": {
      ...
      const evidenceNodes = pickEvidence(seeds, allBoardNodes, seedIds, /* max */ 10);
      const relatedNodes = pickRelated(seeds, allBoardNodes, seedIds, /* max */ 5);
      const limitNodes = seeds.filter((n) => n.limits.length > 0);
...
function pickEvidence(
  ...
  return pool
    ...
    .filter((n) => n.sourceIds.some((s) => seedSourceIds.has(s)) || n.tags.some((t) => seedTags.has(t)))
    .slice(0, max);
}
...
async function loadSourceExcerpts(
  ...
  for (const n of nodes) {
    if (n.sourceChase.length === 0 || n.chunkRefs.length === 0) continue;
    try {
      const exs = getExcerpt(config, n.sourceChase, n.chunkRefs);
      for (const ex of exs) {
        excerpts.push({
...
function buildInstruction(
  ...
  switch (mode) {
    ...
    case "trace":
      ...
      boundaries.requireChunkRef = true;
      break;
    case "expand":
      synthesisLevel = "free";
      break;
  ...
  return {
    ...
    coverageNote: board.seedNodes.length === 0
      ? (totalNodes === 0 ? "wiki is empty" : "no node matches")
      : undefined,
  };
}
