...
## compile — MODE: compile
{
  ...
  "pages": [
    { // 新页面
      "nodeId": "concept/<id>",
      "filePath": "wiki/concepts/<id>.md",
      "frontmatter": { "title": "...", "source": "...", "confidence": 0.9 },
      "body": "..."
    }
  ],
  "updatedPages": [
...
## wiki 文件 frontmatter
- title / source / confidence / hypothesis / hypothesis_title / related / createdAt
...
export function buildVariables(input: VariablesInput): string {
  ...
  const mode = isCompile ? "compile" : isReread ? "reread" : "brainstorm";
  ...
  if (isCompile && input.existingPages && input.existingPages.length > 0) {
