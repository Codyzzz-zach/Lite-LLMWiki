...
import { makeDeepSeekCaller, queryKnowledge } from "../../query/engine.js";
import type {
...
import { type CommandFn, InputLine } from "./InputLine.js";
import { type ChatMessage, MessageLog } from "./MessageLog.js";
import { type Stats, StatusLine } from "./StatusLine.js";
...
export function App() {
	...
	const doFinalSave = useCallback(
		async (
			...
			if (ciList.length > 0) {
				...
				store.saveWikiPage({
					nodeId: daId,
					filePath: `wiki/concepts/${daId}.md`,
					frontmatter: {
						title: `反直觉视角: ${cr.title}`,
			...
			if (cr.humanAnchor) {
				store.saveWikiPage({
					nodeId: cr.humanAnchor.id,
					filePath: `wiki/concepts/${cr.humanAnchor.id}.md`,
					frontmatter: {
						title: cr.humanAnchor.text.slice(0, 80),
						source: cr.materialId,
						confidence: 1.0,
						createdAt: new Date().toISOString(),
					},
	...
	const doCompile = useCallback(async () => {
		...
		if (!src || list.length === 0) {
			add({ kind: "warning", content: "无已确认条目" });
			setPhase("idle");
			return;
		}
		setPhase("compiling");
		...
		try {
			...
			add({
				kind: "result",
				content: `pages: ${nodeDrafts.length} new, ${updatedPages.length} updated`,
			});
			...
			if (updatedPages.length > 0) {
				...
				add({
					kind: "system",
					content: `📄 更新 1/${updatedPages.length}:\n${updatedPages[0]!.body.slice(0, 300)}`,
				});
			...
			add({ kind: "error", content: `编译: ${(e as Error).message}` });
		}
		setBusy(false);
	...
	const showProp = useCallback(() => {
		...
		add({
			kind: "system",
			content: `📄  [${prop.id}/${all}]${rev}  ${prop.claim}`,
		});
		add({
			kind: "ai",
			content: `🤖  ${prop.aiReading}  (Chunk ${prop.chunkRefs.join(", ")})`,
		});
		if (prop.counterIntuitive && prop.counterIntuitiveReason) {
			add({
				kind: "system",
				content: `⚡ 反直觉: ${prop.counterIntuitiveReason}`,
			});
		}
		...
		async (angle: string) => {
			...
			try {
				...
				const rr = await proIngest({
					...
					claim: prop.claim,
					humanAngle: angle,
					targetChunkRefs: prop.chunkRefs,
				});
				...
				if (revised) {
					...
					add({
						kind: "system",
						content: `🤖 [原版] ${origReadingRef.current}`,
					});
					add({
						kind: "system",
						content: `   [a]对齐原版  [r]对齐修订版  [m]再换角度  [s]跳过`,
					});
				} else {
				...
				add({ kind: "error", content: `reread: ${(e as Error).message}` });
				setPhase("waiting_prop");
			}
			setBusy(false);
	...
	const handleCommand: CommandFn = useCallback(
		async (cmd: string, args: string) => {
			...
			if (!cmd) {
				...
				if (phase === "waiting_prop") {
					...
					if (ch === "a all") {
						...
						confirmedRef.current.push({
							...
							threadId: prop.threadId,
							claim: prop.claim,
							aiReading: prop.aiReading,
							chunkRefs: prop.chunkRefs,
						...
						for (let i = curIdx + 1; i < all.length; i++) {
							...
							confirmedRef.current.push({
								...
								claim: rest.claim,
								aiReading: rest.aiReading,
								chunkRefs: rest.chunkRefs,
								revision: 0,
								status: "confirmed",
				...
				if (phase === "waiting_angle") {
					...
					if (!angle) {
						add({ kind: "error", content: "角度不能为空" });
						return;
					}
					mCountRef.current++;
					add({
						kind: "system",
						content: `[Pro] 基于「${angle.slice(0, 40)}…」重读...`,
					});
				...
				if (phase === "waiting_version") {
					...
					if (ch === "a") {
						...
						confirmedRef.current.push({
							...
							threadId: prop.threadId,
							claim: prop.claim,
							aiReading: origReadingRef.current,
							chunkRefs: origChunksRef.current,
							revision: 0,
							status: "confirmed",
							counterIntuitive: prop.counterIntuitive,
							counterIntuitiveReason: prop.counterIntuitiveReason,
						...
						add({
							kind: "result",
							content: `✅ [${prop.id}] 已确认（修订版 r${prop.revision}）`,
						});
						nextProp();
					} else if (ch === "m") {
						if (mCountRef.current >= 3) {
				...
				if (phase === "waiting_update") {
					const ch = raw.trim().toLowerCase();
					const updates = (clientRef as any)._pendingUpdates as
						| any[]
						| undefined;
					...
					if (next < updates.length) {
						...
						add({
							kind: "system",
							content: `📄 更新 ${next + 1}/${updates.length}:\n${updates[next]!.body.slice(0, 300)}`,
						});
						add({ kind: "system", content: "  [a] 应用更新  [s] 跳过" });
					} else {
						// 全部确认完毕，保存
						...
						const list = confirmedRef.current.filter(
			...
			switch (cmd) {
				case "ingest": {
					...
					try {
						if (args.toLowerCase().endsWith(".pdf")) {
							...
							add({
								kind: "result",
								content: `${src.title}  |  ${src.chunks.length} chunks  |  ~${src.totalTokens} tokens`,
							});
							add({
								kind: "system",
								content: '💡 :anchor "问题" 开始 Brainstorm',
							});
							...
							const src: Source = args.toLowerCase().endsWith(".tex")
								? await loadFromTex(args, config)
								: loadFromFile(args, config);
							add({
								kind: "result",
								content: `${src.title}  |  ${src.chunks.length} chunks  |  ~${src.totalTokens} tokens`,
							});
						...
						add({ kind: "error", content: `${(e as Error).message}` });
					}
					break;
				}
				...
				case "anchor": {
					const src = sourceRef.current;
					if (!src) {
						add({ kind: "error", content: "先 :ingest <file>" });
						break;
					}
				...
				case "query": {
					if (!args) {
						add({ kind: "error", content: "需要问题" });
						break;
					}
					try {
						add({ kind: "system", content: "🔍 ..." });
						const caller = makeDeepSeekCaller(config);
				...
				case "status": {
					...
					add({
						kind: "result",
						content: `sources: ${stats.sources}  |  wiki: ${stats.nodes}`,
					});
					break;
				}
				...
				case "help":
					add({
						...
						content: [
							":ingest <file>   加载",
							':anchor "text"   Brainstorm→主线→确认→wiki',
							':query "text"    查询',
							":node <id>       查看",
							":status          统计",
							":clear           清屏",
	...
	return (
		<Box flexDirection="column" height="100%">
			...
			<InputLine
				...
				onRawInput={
					phase !== "idle" && phase !== "loaded" && phase !== "done"
						? (text: string) => handleCommand("", text, text)
						: undefined
				}
				busy={busy}
				phase={phase as any}
