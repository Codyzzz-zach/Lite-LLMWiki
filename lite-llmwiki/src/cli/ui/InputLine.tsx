...
export type CommandFn = (cmd: string, args: string, raw: string) => void | Promise<void>;
...
export function parseCommand(raw: string): { cmd: string; args: string } | null {
  ...
  return { cmd: rest.slice(0, space).toLowerCase(), args: rest.slice(space + 1).trim() };
}
...
export function InputLine({
  ...
  const promptHint = phase === "waiting_focus"
    ? " 输入你的关注方向 (回车直接选 [1])"
    : "";

  return (
    <Box>
      <Text bold color="cyan">{promptLabel} </Text>
      <Text>{input || ""}</Text>
