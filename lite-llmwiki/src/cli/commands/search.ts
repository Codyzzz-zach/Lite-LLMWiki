...
export function registerSearchCommand(program: Command): void {
	program
		...
		.action(
			async (
				query: string,
				options: { json?: boolean; max?: string; includeFailed?: boolean },
			) => {
				const config = loadConfig();
				const maxResults = Number.parseInt(options.max ?? "20", 10) || 20;

				const results = searchWiki(config, query, {
					maxResults,
				...
				console.log(
					`  ${results.length === 0 ? "No matches found." : `Found ${results.length} matches:`}`,
				);
				console.log("");
				...
				for (const m of results) {
					...
					if (m.claim) {
						const snippet =
							m.claim.length > 80 ? m.claim.slice(0, 80) + "…" : m.claim;
