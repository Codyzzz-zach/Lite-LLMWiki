...
export interface RunQueryCliOptions {
	...
	/** 注入 LLM caller（生产环境用 queryKnowledge，测试可 mock）— 可选，CLI 包装层会注入 */
	llmCaller?: (args: {
		question: string;
		board: QueryBoard;
		config: unknown;
...
export async function runQueryCli(
	...
	const maxNodes = Number.parseInt(options.max ?? "5", 10) || 5;

	// ── 验证 llmCaller 必须存在 ──
	if (!options.llmCaller) {
		throw new Error(
			"runQueryCli requires llmCaller — this product must have an API key. Use registerQueryCommand which injects it.",
		);
	}
	...
	const result = await queryKnowledge({
		...
		llmCaller: async (_b, q) => {
			...
			return {
				answer: r.answer,
				modelSynthesis: r.modelSynthesis as ModelSynthesis[],
			};
		},
	});
...
export function registerQueryCommand(program: Command): void {
	program
		.command("query")
		.description(
			"Query the knowledge base with a natural language question (v6 board)",
		)
		...
		.option(
			"--mode <mode>",
			"board mode: ask|trace|expand|compare|challenge|inspire (alias: exact→trace, explore→expand, counter→challenge)",
			"ask",
		)
		...
		.action(async (question: string, options: RunQueryCliOptions) => {
