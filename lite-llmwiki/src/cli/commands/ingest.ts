...
export function registerIngestCommand(program: Command): void {
	program
		.command("ingest")
		.description(
			"Ingest a file or TeX folder — extract → 主线选择 → 逐条确认 → wiki",
		)
		.argument("<path>", "path to .md / .tex file, or a TeX project folder")
		.option("-m, --anchor <text>", "human anchor")
		.option(
			"-t, --thread <id>",
			'skip thread selection: "all" or thread number',
			"",
		)
		.option("--auto", "非交互自动确认（需配合 --policy）")
		.option(
			"--policy <name>",
			"自动确认策略: conservative | balanced | expansive",
			"balanced",
		)
...
export async function runIngestPipeline(
	...
	if (!config.apiKey) {
		const json: IngestJsonOutput = {
			...
			coverage: {
				coveredChunks: 0,
				totalChunks: 0,
				uncoveredReasons: ["DEEPSEEK_API_KEY not set"],
			},
		};
	...
	if (threads.length === 0 || allProps.length === 0) {
		...
		const json: IngestJsonOutput = {
			...
			updated: [],
			skipped: [],
			coverage: computeCoverage(source, allProps, []),
		};
	...
	if (opts.auto && !threadOpt) {
		...
		selectedId = 0;
		const label =
			source.chunks.length < 5 ? "短文档，自动全部" : "--thread all";
		...
		if (threads.some((t) => t.id === n)) {
			...
			const json: IngestJsonOutput = {
				...
				coverage: {
					coveredChunks: 0,
					totalChunks: source.chunks.length,
					uncoveredReasons: [`--thread ${threadOpt} invalid`],
				},
			};
			return { ok: false, exitCode: 1, json };
		}
	...
	if (opts.auto) {
		...
		for (const prop of targetProps) {
			...
			if (result.accept) {
				...
				out(
					`  ✅ [${prop.id}] auto-confirmed (${prop.kind ?? "?"} conf=${(prop.confidence ?? 0).toFixed(2)})`,
				);
			} else {
				skippedReasons.push({
					propId: prop.id,
		...
		for (let pi = 0; pi < targetProps.length; pi++) {
			...
			while (true) {
				...
				out(
					`  ─── [${current.id}/${targetProps.length}]${revTag}  (剩余 ${remainCount}) ───`,
				);
				out(`  📄  ${current.claim}`);
				...
				if (choice === "a all") {
					confirmed.push({
						...
						claim: current.claim,
						aiReading: current.aiReading,
						chunkRefs: current.chunkRefs,
				...
				if (choice === "m") {
					...
					out(
						`  [Pro] 基于「${angle.slice(0, 40)}…」重读 Chunk ${current.chunkRefs.join(", ")}...`,
					);
					...
					while (true) {
						...
						if (p === "a") {
							confirmed.push({
								...
								threadId: current.threadId,
								claim: current.claim,
								aiReading: originalReading,
								chunkRefs: originalChunks,
								revision: 0,
								status: "confirmed",
								counterIntuitive: current.counterIntuitive,
								counterIntuitiveReason: current.counterIntuitiveReason,
						...
						if (p === "r") {
							confirmed.push({
								...
								threadId: prop.threadId,
								claim: prop.claim,
								aiReading: revised.aiReading,
								chunkRefs: revised.chunkRefs,
	...
	try {
		...
		out(
			`        pages:   ${nodeDrafts.length} new, ${updatedPages.length} updated\n`,
		);
		...
		if (opts.dryRun) {
			...
			const chasePath = join(
				config.rawDir,
				"chase",
				`${source.id.replace(/[\/:]/g, "_")}.md`,
			);
			out(`        chase: ${chasePath}`);
			...
			auditSummary = {
				structure: {
					...
					failed:
						...
						structureResult.summary.verifiedNodes,
				},
			};

			// 必须有 API key（本产品不运行在无 key 模式）
			try {
				const auditClient = client ?? new DeepSeekClient(config);
				const semanticResult = await import(
					"../../knowledge/semantic-audit.js"
				).then((m) =>
					m.runSemanticAudit(config, {
				...
				out(
					`  🔍  audit: ${semanticResult.ok ? "passed" : "issues found"} (semantic score: ${semanticResult.summary.averageScore})`,
				);
			} catch (semanticErr) {
				...
				out(
					`  🔍  audit: structure passed, semantic skipped (API error: ${semanticMessage.slice(0, 80)})`,
				);
				auditSummary.semantic = {
					ok: false,
					averageScore: 0,
		...
		const json: IngestJsonOutput = {
			...
			coverage: {
				coveredChunks: 0,
				totalChunks: source.chunks.length,
				uncoveredReasons: [(err as Error).message],
			},
		};
		return { ok: false, exitCode: 1, json };
	}
...
function buildCounterNode(
	...
	const ciList = propositions.filter(
		(c) => c.counterIntuitive && c.status === "confirmed",
	);
	if (ciList.length === 0) return null;

	const suffix = compileResult.materialId.slice(-8);
	const nodeId = `counter-${suffix}`;
	const chunkRefs = [...new Set(ciList.flatMap((c) => c.chunkRefs))].sort(
		(a, b) => a - b,
	);
