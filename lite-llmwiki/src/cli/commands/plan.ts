...
export function registerPlanCommand(program: Command): void {
  program
    .command("plan")
    .description("Extract propositions and coverage from a source — dry-run, no wiki writes")
    .argument("<path>", "path to .md / .tex / .pdf file, or a TeX project folder")
...
async function runPlan(opts: PlanOptions): Promise<void> {
  ...
  try { stat = statSync(opts.file); } catch { /* not found */ }
  ...
  let source;
  try {
    source = stat?.isDirectory() || ext === ".tex"
    ...
    const result = { ok: false, error: `Failed to load source: ${(err as Error).message}` };
  ...
  try {
    ...
    const result = { ok: false, error: `Extract failed: ${(err as Error).message}` };
    console.log(opts.json ? JSON.stringify(result) : `  ❌  ${result.error}`);
    process.exit(1);
  }
  ...
  if (opts.json) {
    ...
    console.log(`      type: ${source.type}  |  chunks: ${totalChunks}  |  tokens: ~${source.totalTokens}\n`);
