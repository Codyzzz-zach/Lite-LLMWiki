...
/** 命题级 chase chunk —— 替代旧的段落级 ChaseChunk */
export interface ChaseProp {
	index: number;
	text: string;
	marker: string;
}
...
export function normalizeBoardMode(
...
export interface WikiFrontmatter {
	...
	auditVerdict?: SemanticJudgeVerdict;
	/** v6 回流来源标记——回流节点无 chase 命题原文，可信度低于编译节点 */
	reflowOrigin?: string;
	/** v6 图谱边——frontmatter 是边的主数据源 */
	edges?: GraphEdge[];
	/** 置信度 0-1 */
	confidence?: number;
	/** v5 节点状态 */
...
export interface ValidatedWikiFrontmatter extends WikiFrontmatter {
	...
	createdAt: string;
	updatedAt: string;
	// v6 字段（缺省时由 normalizeFrontmatter 给出合理默认值）
	auditStatus: AuditStatus;
	auditScore?: number;
	claimType?: ClaimType;
	inferenceLevel?: InferenceLevel;
...
export interface SemanticJudgeVerdict {
	...
	/** 证据锚点——judge 所依据的命题原文引用（prop N: "原文"），Layer 1 防御核心 */
	anchor?: string;
	issues: string[];
}
